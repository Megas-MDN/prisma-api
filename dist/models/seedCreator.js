...
const writeTable = async (table, value) => {
    ...
    return promises_1.default.writeFile(`./prisma/seeds/${tableNameCamelCase}.ts`, `export const ${tableNameCamelCase} = ${JSON.stringify(value)};`);
};
...
const findAndRefind = async (tables) => {
    ...
    tables.map((table) => _1.default[table].findMany()));
